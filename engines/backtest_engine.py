...
class Trade:
    ...
    exit_reason: str = ""
    tc_cost_pct: float = 0.0   # round-trip TC in % (set after entry)
    ...
    def pnl_pct(self) -> float | None:
        if self.exit_price is None:
            return None
        gross = (self.exit_price - self.entry_price) / self.entry_price * 100.0
        return gross - self.tc_cost_pct   # already includes both legs

    def set_tc(self, tc_pct: float):
        """Store round-trip TC so pnl_pct is net."""
        self.tc_cost_pct = tc_pct * 2.0   # entry + exit
...
async def run_backtest_streaming(
    ...
    tc_pct: float = 0.1,        # one-way TC as % of trade value (default: 0.1% = Binance spot maker)
    ...
    Parameters
    ----------
    df              : MCb-calculated DataFrame
    strategy        : Instantiated MCBStrategy
    replay_delay_ms : ms to sleep between frames
    tc_pct          : One-way transaction cost as % (e.g. 0.1 = 0.1%).
                      Applied on both entry and exit, so round-trip = 2 × tc_pct.
    """
    ...
    rt_tc = tc_pct * 2.0   # round-trip cost %
