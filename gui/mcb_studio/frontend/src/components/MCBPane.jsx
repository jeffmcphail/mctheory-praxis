...
const MCBPane = forwardRef(function MCBPane({ height = 180, visibleRange }, ref) {
  const canvasRef  = useRef(null)
  const barsRef    = useRef([])
  const rangeRef   = useRef(null)   // current {from, to} logical range
  const animRef    = useRef(null)
  const dirtyRef   = useRef(false)

  // Update range when prop changes
  useEffect(() => {
    rangeRef.current = visibleRange || null
  ...
  const draw = useCallback(() => {
    ...
    // OB/OS zone fill
    ctx.fillStyle = C.ob
    ctx.fillRect(0, 0, w, yPx(OB, h))
    ctx.fillStyle = C.os
    ctx.fillRect(0, yPx(OS, h), w, h - yPx(OS, h))

    if (bars.length < 2) return

    // Determine visible slice from logical range
    ...
    // Legend
    const legendItems = [
      { color: C.wt1, label: 'WT1' }, { color: C.wt2, label: 'WT2' },
      { color: C.rsi, label: 'RSI' }, { color: C.mfiBull, label: 'MFI+' },
      { color: C.dotBuy, label: '● Buy' }, { color: C.dotGold, label: '● Gold' },
      { color: C.dotEntry, label: '▲ Entry' }, { color: C.dotExit, label: '▼ Exit' },
    ]
