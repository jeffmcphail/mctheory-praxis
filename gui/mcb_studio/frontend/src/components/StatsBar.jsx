...
export default function StatsBar({ stats, progress, status, running, error, tcPct }) {
  ...
  return (
    <div style={S.bar}>
      ...
      <span style={{
        background: tcPct > 0 ? 'rgba(255,183,77,0.15)' : '#1e2430',
        border: `1px solid ${tcPct > 0 ? '#ffb74d' : '#2a3248'}`,
        color: tcPct > 0 ? '#ffb74d' : '#5d6b8a',
        borderRadius: 3, padding: '2px 8px', fontSize: 11, fontWeight: 600,
      }}>
        {tcPct > 0 ? `TC ${tcPct}%/leg` : 'No TC'}
      </span>
