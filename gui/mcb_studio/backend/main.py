...
from batch_engine import run_batch, BatchRunConfig
...
class BacktestRequest(BaseModel):
    ...
    tc_pct: float = 0.1       # one-way TC % (0.1 = Binance spot maker)
...
class BatchRequest(BaseModel):
    runs: list[dict]   # list of BatchRunConfig dicts


@app.post("/api/batch/start")
def api_batch_start(req: BatchRequest):
    session_id = str(uuid.uuid4())
    sessions[session_id] = {"type": "batch", "request": req.model_dump(), "status": "pending"}
    return {"session_id": session_id}


@app.websocket("/ws/batch/{session_id}")
async def ws_batch(websocket: WebSocket, session_id: str):
    ...
    req_data = sessions[session_id]["request"]
    runs = [BatchRunConfig(**r) for r in req_data["runs"]]

    try:
        async for frame in run_batch(runs):
...
async def ws_backtest(websocket: WebSocket, session_id: str):
    ...
    speed_ref = {"ms": req.replay_speed_ms}

    async def listen_for_control():
        """Background task: receive control messages (e.g. set_speed) from client."""
        try:
            while True:
                msg = await websocket.receive_json()
                if msg.get("type") == "set_speed":
                    speed_ref["ms"] = int(msg.get("ms", speed_ref["ms"]))
        except Exception:
            pass
    ...
    try:
        ...
        # Start control listener as background task
        control_task = asyncio.create_task(listen_for_control())

        async for frame in run_backtest_streaming(df, strategy, req.replay_speed_ms, req.tc_pct, speed_ref):
            await _send(websocket, frame)
